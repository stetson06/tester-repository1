...
Assuming the "gen-ai" virtual environment already resides in Anaconda Navigator (drop-down menu at the top ribbon in the dialog box after "on"), we activate this option for this GenAI projects using the code lines below in the Anaconda prompt. If it is not, then please refer back to the RAG project (look for the same "Virtual Environment" section header as above) to get this VE established.
