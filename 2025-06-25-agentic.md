...
We frrst must tell LangChain which part of the database it is allowed to interact with by wrapping our SA engine inside LangChain's SQLDatabase class. We specifiy the engine to enable a live connection to the Postgres SQL database. The engine includes only the two tables in the database it needs, for streamlined processing! It allows LangChain to go and fetch n = 5 number of rows from each of the two tables, which get included in the schema text it sends to the LLM. 

Through all this, the model can see real column names and data types when figuring out how to write queries that will answer the user's question(s).
